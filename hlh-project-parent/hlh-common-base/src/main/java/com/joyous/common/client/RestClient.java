...
import com.joyous.common.trance.TraceContext;
import com.joyous.common.util.JSONMapper;
import com.joyous.common.exception.ExceptionCode;
...
public class RestClient {
    ...
    private RestClient() {
        ...
        ServiceLoader<ClientRequestFilter> serviceLoader = ServiceLoader.load(ClientRequestFilter.class);
		Iterator<ClientRequestFilter> iterators = serviceLoader.iterator();
		if (iterators.hasNext()) {
			client.register(iterators.next());
		}
    }
    ...
    public <T> T get(String url, Class<T> responseType) {
    	...
    	try{
	        ResteasyWebTarget target = getClient().target(url);
	        response = target.request().headers(
					TraceContext.getContext2MultivaluedMap()).get();
